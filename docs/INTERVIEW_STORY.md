...
5. **Prove it.** A sealed harness injects 7 break classes (exact, rounding,
   fee mismatch, orphan, duplicate, out-of-order, late correction) with an
   answer key the matcher never sees: P=R=F1=1.0, zero false positives on 2000×3.
   `make demo` runs it in ~10 seconds, no infra. Real thiru scale (12.6M, 89.98% on ~10% mix, 90.65% branch coverage) proves it at size.

## Live demo (clean checkout, <5 min with docker)

```bash
docker compose up -d          # Redpanda + MinIO + Nessie
make demo                     # sealed F1=1.0 (2000×3) + hardware fingerprint
ALLOW_DESTRUCTIVE_SEED=1 python -m src.pipeline --date 2026-09-04 --demo   # synthetic (seeds webhooks)
python -m src.pipeline --date 2026-09-04               # real PG file (fail-closed, no synthesis)
python -m src.pipeline --date 2026-09-04 --demo --messy  # full exception mix + LATE lifecycle
```
...
- *How do you know a slowdown is real?* Baselines carry a hardware fingerprint; the regression gate warns on a new machine and only fails on metric movement (>15% throughput drop, >20% p99 rise). Real-data benchmarks are isolated (`--real` → `results_real.json`, zero synthetic overwrite).
